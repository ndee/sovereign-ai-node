...
- optional requested node name when you want an exact hostname and the relay reports it is available

`relay` mode does not require a user-managed domain or inbound port forwarding.
Node hostname assignment in relay mode can use a requested exact node name when available; otherwise the relay auto-generates a unique name. Existing relay assignments stay immutable on later reconfigure runs.
