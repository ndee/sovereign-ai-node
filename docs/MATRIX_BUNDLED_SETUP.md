...
- optional `relay.requestedSlug` for seeded/automated relay flows that want a stable preferred hostname slug; the installer may still fall back to a generated slug when the requested one is unavailable
...
7. Apply the bundled Matrix avatars that exist on disk for the service bot, alert room, and managed agents.
8. Create private alert room and invite operator + required core agents.
9. Persist homeserver URL, mapped Matrix identities/tokens, alert room ID, and applied avatar hashes into Sovereign/OpenClaw config.
...
- apply `service-bot.png` to the service bot when present at the repo root
- apply `alert-room.png` to the alert room when present at the repo root
- for each managed agent, prefer `bots/<id>/avatar.png`; when absent, fall back to the repo-root `service-bot.png`
- persist avatar hashes so installer re-runs can skip unchanged uploads
