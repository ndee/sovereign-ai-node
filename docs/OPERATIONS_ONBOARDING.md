...
- the bundled install path installs Docker Engine + the Compose plugin automatically when the host does not already provide them
...
1. Run preflight checks (ports, disk, DNS resolution, clock) and detect whether the host already has a usable container runtime.
2. Install Docker Engine + the Compose plugin automatically when they are missing.
