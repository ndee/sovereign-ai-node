...
- Relay mode: no user-managed domain required
  ...
  - request-file-driven or seeded relay flows may carry `relay.requestedSlug`; the installer should honor it when available and otherwise fall back to a generated relay slug
- Mail server details for the mailbox Mail Sentinel should watch: protocol (IMAP or POP3), hostname, port, TLS mode, email address / username (usually your full email address, depending on your provider), and password or app password
- Outbound network access to the mail server, model provider(s), and Matrix endpoints
...
Settings entered during installation do not require a reinstall to change.
The operator can use the local web UI (Pro) or the CLI:

| Setting | Editable after install | How |
|---|---|---|
| Mail protocol (IMAP ↔ POP3), server, port, TLS, email address / username, password, IMAP folder | yes | web UI *Settings → Mail connection*, or `sovereign-node reconfigure imap …` |
| OpenRouter API key, default model | yes | web UI *Settings → LLM provider*, or `sovereign-node reconfigure openrouter …` |
| Matrix federation | yes | `sovereign-node reconfigure matrix --federation/--no-federation` |
| Mail Sentinel poll interval, lookback, reminders, digest | yes | `sovereign-node mail-sentinels update <id> …` |
| Node name (relay slug), homeserver domain, Matrix URL | **no** (read-only) | the homeserver identity, relay assignment and TLS certificate are keyed to it; changing it is a re-install/migration |

Mail changes follow **validate → persist → apply**: the new connection is
tested with the new credentials first, and only a successful test replaces
the stored configuration. A failed test leaves the last working connection
active. Existing secrets are never shown back; replacing one requires
entering the new value.

### IMAP vs POP3

IMAP is the recommended protocol. POP3 is supported for providers that only
offer POP3, with these differences:

- Mail Sentinel is read-only over both protocols. The POP3 client has no
  delete command at all, so messages are never removed from the server.
- POP3 has no folders; the *folder* setting is ignored and always `INBOX`.
- POP3 has no server-side search or stable numeric IDs. The node keeps a
  small per-instance index (`/var/lib/sovereign-node/pop3-index/`, mode
  `0600`) mapping each message's UIDL to a stable ID, so restarts do not
  re-process mail. Losing the index resets the watermark; Message-ID
  deduplication in the bot state still prevents duplicate alerts.
- The first scan of a large mailbox is bounded: headers are fetched newest
  first and older mail outside the lookback window is recorded without
  being fetched or reported.
- Only TLS (implicit on 995, or STLS on 110) is accepted for remote hosts.
