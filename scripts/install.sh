...
DEFAULT_RELAY_NODE_NAME="${SOVEREIGN_NODE_RELAY_NODE_NAME:-}"
...
prompt_relay_node_name() {
  local default input normalized
  default="${1:-}"
  while true; do
    input="$(prompt_value "Relay node name (optional; type auto for random)" "$default")"
    if [[ "${input,,}" == "auto" ]]; then
      printf '%s' ""
      return 0
    fi
    ...
    normalized="$(sanitize_relay_node_name "$input")"
    if [[ -z "$normalized" ]]; then
      ui_warn "Relay node names must include at least one lowercase letter or number."
      default=""
      continue
    fi
    if [[ "$normalized" != "$input" ]]; then
      ui_info "Using relay node name: ${normalized}"
    fi
    printf '%s' "$normalized"
    return 0
  done
...
const runtimeRelayPublicBaseUrl =
  runtimeRelayEnabled && typeof runtimeRelay.publicBaseUrl === "string"
    ...
    : "";
const runtimeRelayNodeName =
  runtimeRelayHostname.length > 0
    ? runtimeRelayHostname.split(".")[0] ?? ""
    : "";
...
emit(
  "DEFAULT_RELAY_NODE_NAME",
  runtimeRelayNodeName || relay.requestedNodeName || "",
);
...
if (connectivityMode === "relay") {
  ...
  if ((process.env.SN_RELAY_NODE_NAME || "").trim().length > 0) {
    req.relay.requestedNodeName = process.env.SN_RELAY_NODE_NAME.trim();
  }
}
...
review_install_request() {
  ...
  if [[ "${SN_CONNECTIVITY_MODE}" == "relay" ]]; then
    ...
    if [[ -n "${SN_RELAY_NODE_NAME:-}" ]]; then
      ui_info "Requested node name: ${SN_RELAY_NODE_NAME}"
      ui_info "Node name availability will be checked during relay enrollment."
    else
...
run_install_wizard() {
  ...
  local relay_control_url relay_enrollment_token relay_node_name
  ...
  relay_node_name="$DEFAULT_RELAY_NODE_NAME"
  ...
  if [[ "$connectivity_mode" == "relay" ]]; then
    ...
    relay_node_name="$(prompt_relay_node_name "$relay_node_name")"
    if [[ "${relay_control_url%/}" != "$DEFAULT_MANAGED_RELAY_CONTROL_URL" ]]; then
  ...
  export SN_RELAY_NODE_NAME="${relay_node_name:-}"
