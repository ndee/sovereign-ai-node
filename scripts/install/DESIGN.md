...
- A verified Node release artifact can be supplied with `--node-artifact` plus `--node-artifact-manifest`. That mode validates and atomically stages the compiled runtime, installs only its locked production dependencies with scripts disabled, and skips source cloning/building.
